...
%%
%Defining Constants
global rho gamma_M gamma_m E1 E2 E3 c2 c L1 LL alpha Nu mil G0
rho = 1;
gamma_M = 1;
gamma_m = 2;
E1= 10;
...
LL = 0.1%0.3; % Goes into initial condition for Phi
...
Phi_Stiffness = 100 %This is the Kappa(of DPFM) in SPFM


% M3 = [-0.9,-0.75,-0.6,-0.45,-0.3,-0.15,0]%linspace(-1,1.5,51)% s_dot/c3
% M3 = [0,0.15,0.3,0.45,0.6,0.75,0.9,1.05]
% M3 = 1.35
M3 = [-0.9,-0.75,-0.6,-0.45,-0.3,-0.15,0,0.15,0.3,0.45,0.6,0.75,0.9,1.05]

Length = [20,1,1,1,1]; %red:30,10; blue:20,20, magenta: 10,20; green: 20,10
EE = [10,1,1,1,1];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for k = 1:length(Phi_Stiffness)
    Phi_Stiff = Phi_Stiffness(k);%viscosity
    for p = 1:length(M3)
        %% Defining Grid
        if M3(p)<1
            if Phi_Stiffness(k)==Phi_Stiffness(1)
                L = Length(1)
                E = EE(1)
            elseif Phi_Stiffness(k)==Phi_Stiffness(2)
                L = Length(1)
                E = EE(1)
            elseif Phi_Stiffness(k)==Phi_Stiffness(3)
                L = Length(1)
                E = EE(1)
            end
        elseif M3(p) >= 1
...
%             phi0(i) = 0.5*H(xi(i)-position)+0.2;
        ...
        %% Solving
        [F] =  @(initialVals) travellingWaveEqn5(nx,delta_x,Phi_Stiff,M3(p),initialVals)
        %% fmincon
        A = [];
        zcol = zeros(nx+1,1);
        ...
        options = optimoptions('fmincon','Display','iter','MaxFunctionEvaluations',100000000,'MaxIterations',100000);
...
%                 [Ux_Phi,FF] = fminunc(F,initialValues, options);
        %% fminsearch
