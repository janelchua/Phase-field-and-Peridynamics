function [TotalResidual,gradFF] = travellingWaveEqn5(nx,delta_x,Phi_Stiffness,M,a)
%% Model2wDamping
%%Solving for Ux
%% Global Constants
...
for n = 1:nx % All the nodes
    ...
    % f
    firstTerm = c2*gamma_m - c2*gamma_M + Phi_Stiffness.*(Phi(n).^2).*(2.*Phi(n) - 2) + (E1*gamma_M^2)/2 + ...
        (E2*gamma_M^2)/2 - (E2*gamma_m^2)/2 - ...
        (E3*gamma_m^2)/2 + Phi_Stiffness.*2.*Phi(n).*(Phi(n) - 1).^2 + ...
        (Ux(n).^2.*(tanh((Phi(n) - 1/2)./L1).^2 - 1).*(E1 - E3))./(4*L1);
    f = (-(firstTerm) + alpha.*Phi_xx(n));
